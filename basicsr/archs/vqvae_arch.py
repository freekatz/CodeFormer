...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, torch.Tensor, List[float]]:
        dtype = f_BChw.dtype
        ...
        # margin = pn*pn / 100
        if ret_usages:
            usages = torch.tensor([(self.ema_vocab_hit_SV[si] >= margin).float().mean().item() * 100 for si, pn in
                      enumerate(self.v_patch_nums)], device=f_BChw.device).mean()
        else:
            usages = None
        return f_hat, mean_vq_loss, usages
...
class VQVAE(nn.Module):
    ...
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        f_hat, vq_loss, usages = self.quantize(self.quant_conv(self.encoder(inp)), ret_usages=ret_usages)
