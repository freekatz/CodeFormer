...
class CodeFormerIdxModel2(SRModel):
    ...
    def init_training_settings(self):
        ...
        elif self.opt.get('network_vqvae', None) is not None:
            self.hq_vqvae_fix = build_network(self.opt['network_vqvae']).to(self.device)
            self.hq_vqvae_fix.eval()
            self.generate_idx_gt = True
            for param in self.hq_vqvae_fix.parameters():
                param.requires_grad = False
        else:
            raise NotImplementedError(f'Shoule have network_vqvae config or pre-calculated latent code.')
